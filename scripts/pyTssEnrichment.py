...
#只能应用于双端序列
#enrichment为the first 200bp范围内的
#insertions的cumulative长度/insertions个数的均值
import os
...
def asn_mat(val,mat,s_int,e_int,t,i,weight):
    #mat是一个1000行4000列的全零矩阵
    #i是某一plus_TSS.tsv或某一minus_TSS.tsv行索引
    #weight为1
...
def sub_Mat(start):
#逐每个plus_TSS.tsv或minus_TSS.tsv的块运行,一共options.c块
    # initialize data matrix
    mat = np.zeros([rows,cols])
    # 创建了一个1000行4000列的全零矩阵
...
#读取plus_TSS.tsv或minus_TSS.tsv,此时会跳过空行和以#开头的注释行
